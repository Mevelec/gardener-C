...
namespace InstrumentorAPI {
	...
	void Instrumentor::beginSession(const std::string& name, const std::string& filepath)
	{
		...
		if (this->currentSession) {
			...
			// profiling output.
			if (LoggerAPI::Logger::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
				CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, this->currentSession->name);
			}
